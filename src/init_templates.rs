...
pub const KARPATHY_COGS_TOML: &str = r#"# cogs vault configuration — Karpathy-style three-layer wiki.
...
[embeddings]
# Flip to true once an embedding provider is available (Ollama by default:
# `ollama pull nomic-embed-text`). Powers semantic search, the similarity
# overlay in the graph, and similar_notes/semantic_search MCP tools.
enabled = false
provider = "ollama"
...
pub const KARPATHY_AGENTS_MD: &str = r#"# AGENTS.md — operating manual for this wiki
...
pub const KARPATHY_RAW_README: &str = r#"# raw/ — immutable source layer
...
pub const KARPATHY_WIKI_INDEX: &str = r#"---
title: Wiki index
kind: moc
---
...
pub const KARPATHY_WIKI_LOG: &str = r#"# Log

Append-only audit trail. One entry per writing session:

`## [YYYY-MM-DD] <ingest|lint|qa> | <subject>` followed by pages touched.
"#;

pub const KARPATHY_ZED_SETTINGS: &str = r#"{
  // cogs wiring for Zed. Requires the cogs extension (or set
  // lsp.cogs.binary.path to an absolute path if `cogs` isn't on PATH).
  "context_servers": {
    "cogs": {
      "command": "cogs",
      "args": ["mcp"]
  ...
  "languages": {
    "Markdown": {
      "language_servers": ["cogs", "..."]
    }
  }
}
"#;

pub const KARPATHY_GITIGNORE: &str = r#"# cogs graph cache — regenerable, never commit
.cogs/
