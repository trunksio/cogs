//! Pure core of the cogs ingest pipeline: the typed model outputs (with
//! tolerant deserialization for the shapes local models actually produce),
//! JSON reply repair, small text helpers, and the hard validators that gate
//! every ingest.
//!
//! No graph, LLM, or filesystem dependencies — this crate compiles to wasm32
//! so browser-side ingest (Cogitarium, via cogs-wasm) runs the SAME
//! validators as native `cogs ingest`. The validators are the ingest quality
//! story; they exist exactly once, here.

pub mod jsonrepair;
pub mod text;
pub mod validators;

pub use jsonrepair::{extract_json, parse_json_reply, repair_truncated_json};
